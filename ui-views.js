...
function abrirModalCorrecaoProventosOrfaos(proventosOrfaos) {
    ...
    let tableHtml = `
        <p style="margin-bottom: 15px; color: #555;">Estes proventos estão registrados na base, mas o sistema acusa que você não tinha cotas suficientes na Data Com. <strong>Eles devem ser removidos para evitar erros de cálculo.</strong></p>
        <table class="dashboard-table tabela-correcao-orfaos">
            <thead>
                <tr>
                    ...
                    <th>Data Pag.</th>
                    <th class="numero">Valor Total (R$)</th>
                    <th class="controles-col text-center">Ações</th>
                </tr>
            </thead>
            <tbody>
    `;
    ...
    proventosOrfaos.forEach(provento => {
        const dataComFmt = provento.dataCom ? new Date(provento.dataCom + 'T12:00:00').toLocaleDateString('pt-BR') : '-';
        const dataPagFmt = provento.dataPagamento ? new Date(provento.dataPagamento + 'T12:00:00').toLocaleDateString('pt-BR') : '-';
        const valorFmt = formatarMoeda(provento.valorTotalRecebido || 0);

        tableHtml += `
            <tr class="provento-correcao-row">
                <td class="correcao-ativo-container"><strong>${provento.ticker}</strong></td>
                <td>${provento.tipo || 'N/D'}</td>
                <td>${dataComFmt}</td>
                <td>${dataPagFmt}</td>
                <td class="numero"><strong>${valorFmt}</strong></td>
                <td class="controles-col text-center">
                    <i class="fas fa-trash acao-btn delete" title="Excluir este provento" onclick="excluirProventoOrfaoIndividual(${provento.id})" style="cursor: pointer;"></i>
                </td>
            </tr>
        `;
    });
    ...
    tableHtml += `
        <div style="margin-top: 25px; display: flex; justify-content: flex-end; gap: 10px;">
            <button type="button" class="btn btn-secondary" onclick="fecharModal('modal-corrigir-proventos-orfaos')">Cancelar</button>
            <button type="button" class="btn btn-danger" onclick="excluirTodosProventosOrfaos()">
    ...
    const modalCorrigirProventosOrfaos = document.getElementById('modal-corrigir-proventos-orfaos');
    if (modalCorrigirProventosOrfaos) {
